...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (activeTab === "phone") {
      setErrorMsg("Phone login is currently a placeholder. Please use Email Login.");
      return;
    }
    ...
    <div className="flex min-h-screen w-full bg-white select-none overflow-hidden font-sans">
      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-900 text-white p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="my-auto max-w-lg z-10 space-y-6">
          ...
          >
            Invest in Nifty 50, ETFs, and Options
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Join PaperTrade today and start simulated investing with ₹1,00,000 virtual cash and zero risk.
          </motion.p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-slate-50/50 min-h-screen">
        <motion.div 
          ...
          className="w-full max-w-md bg-white rounded-3xl p-8 lg:p-10 shadow-xl border border-slate-100 relative"
        >
          {/* Mobile logo display */}
          ...
          <div className="space-y-2 mb-8">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Log in to PaperTrade</h2>
            <p className="text-sm text-slate-500 font-medium">Invest smart, practice risk-free</p>
          </div>
          ...
          <div className="flex border-b border-slate-100 mb-6">
            <button
              ...
              className={`pb-3 text-sm font-semibold relative px-2 transition-colors duration-200 ${
                activeTab === "phone" ? "text-blue-600" : "text-slate-400 hover:text-slate-600"
              }`}
            >
              Phone Login
              {activeTab === "phone" && (
            ...
            <button
              ...
              className={`pb-3 text-sm font-semibold relative px-2 ml-6 transition-colors duration-200 ${
                activeTab === "email" ? "text-blue-600" : "text-slate-400 hover:text-slate-600"
              }`}
            >
              Email Login
              {activeTab === "email" && (
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            {activeTab === "email" ? (
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 pl-12 pr-12 text-sm font-medium text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition-all"
                    />
                    <button
                      type="button"
              ...
              // Phone Inputs (Demo Placeholder)
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Phone Number</label>
                  <div className="flex space-x-2">
                    <div className="w-24 shrink-0 relative">
                      <select className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 px-3 text-sm font-medium text-slate-900 appearance-none focus:outline-none focus:border-blue-500 focus:bg-white">
                        <option>+91 (IN)</option>
                        <option>+1 (US)</option>
                        <option>+44 (UK)</option>
                      </select>
                      <span className="absolute right-3 top-4 pointer-events-none border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-slate-400"></span>
                    </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-primary hover:bg-primary-dark text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-blue-500/20 active:scale-[0.98] disabled:opacity-75 disabled:pointer-events-none flex items-center justify-center text-base mt-2"
            >
              {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => { setErrorMsg("Passkey login is a mockup feature."); }}
